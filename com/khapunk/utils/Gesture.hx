...
class Gesture
{
	...
	public static function update()
	{
		...
		if (_lastTap > 0) _lastTap = Math.max(0, _lastTap - KP.elapsed / doubleTapTime);
		...
		switch (mode)
		{
			...
			case SINGLE_TOUCH:
			{
				...
				{
					var touch:Touch = getTouch(touches, touchOrder, 0);
					var dist = KP.distance(touch.startX, touch.startY, touch.x, touch.y);
					if (dist > deadZone)
					{
						mode = SINGLE_MOVE;
					}
			...
			case MULTI_TOUCH:
			{
				...
				else
				{
					...
					if (t1 != null && t2 != null)
					{
						var d1 = KP.distance(t1.startX, t1.startY, t1.x, t1.y);
						var d2 = KP.distance(t2.startX, t2.startY, t2.x, t2.y);
						if (d1 > deadZone && d2 > deadZone)
						{
							...
							var inner = KP.distance(t1.startX, t1.startY, t2.startX, t2.startY);
							var outer = KP.distance(t1.x, t1.y, t2.x, t2.y);
