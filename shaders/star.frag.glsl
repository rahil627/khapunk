...
void kore( void ) {
	...
	vec2 position = ( texCoord.xy / resolution.xy ) - pos;
	...
	for (float ray = 0.0; ray < 60.0; ray += 1.0) {
		...
		float rayang = rand(5234, ray)*6.2+(pos.x+time*0.01)*10.0*(rand(2546, ray)-rand(5785, ray))-pos.y*10.0*(rand(3545, ray)-rand(5467, ray));
